...
impl CPUUsage {
    ...
    pub fn get_cpu_info(&mut self) -> Result<f64, io::Error> {
        ...
        let mut cpu_vec: Vec<&str> = cpu_info.split(' ').collect();

        let cpu_values: Vec<f64> = cpu_vec.drain(2..).map(|x| x.parse().unwrap()).collect();
    ...
    fn calculate_current_cpu_usage(&mut self, cpu_vec: &Vec<f64>) -> f64 {
        ...
        let current_cpu =
            (current_usage - self.prev_usage) * 100.0 / (current_time - self.prev_time);
        self.prev_usage = current_usage;
        self.prev_time = current_time;
...
impl MemInfo {
    ...
    pub fn extract_kb_info(&self, line: Vec<&str>) -> f64 {
        let line_vec: Vec<&str> = line[0].split_whitespace().collect();
        let kb: f64 = line_vec[1].parse().unwrap();
        kb
    }
    ...
    pub fn get_mem_info(&mut self) -> Result<f64, io::Error> {
        ...
        let mem_total_line: Vec<&str> = mem_file
            .lines()
            .filter(|line| line.contains("MemTotal"))
            .collect();
        let mem_free_line: Vec<&str> = mem_file
            .lines()
...
impl ProcessInfo {
    ...
    pub fn get_mem_data(&mut self, contents: &str) -> Result<(), io::Error> {
        let contents_array: Vec<&str> = contents.split_whitespace().collect();
        ...
        let total_time = utime + stime;
        let mem_data = Process::new(pid, process_name, state, utime, stime);
    ...
    pub fn add_to_processes(&mut self, mut process: Process) -> Result<(), io::Error> {
        let found_process = self.processes.iter().find(|p| p.pid == process.pid);
        match found_process {
            Some(p) => {
                ...
                let filtered_processes: Vec<Process> = self
                    .processes
                    .iter()
                    .cloned()
